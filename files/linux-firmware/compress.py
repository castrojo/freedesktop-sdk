...
link_re = re.compile(
    r'^Link:\s*(?P<quote1>"?)(?P<link_name>.*)(?P=quote1)(?<!\s)\s+->\s+(?P<quote2>"?)(?P<target>.*)(?P=quote2)(?<!\s)\s*$'
)
    ...
    while line := whence.readline():
        ...
        if mlink:
            ...
            real_target = os.path.normpath(
                os.path.join(os.path.dirname(link_name), target)
            )
            ...
            if os.path.islink(name):
                ...
                subprocess.run(["xz", "-C", "crc32", "-T0", name], check=True)
            else:
                # Already compressed.
